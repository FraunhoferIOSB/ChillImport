  ...
  $.ajax({
    ...
    success : function(response) {
      ...
      for (var i = 0; i < jsonparsed.length; i++) {
        var option =
            new Option(jsonparsed[i].name + " (" + jsonparsed[i].frostId + ")",
...
function createDS() {
  ...
  for (var i = 0; i < $rows.length; i++) {
    ...
    if (selectedType == null || selectedType == "") {
      notifier.alert(
          'Datastream could not be created, at least one Observaton Type is invalid');
      return false;
    }
    types.push(selectedType);
    selectedProp = $rows.eq(i)
                       .find(".selectProperties option:selected")
                       .attr("data-value");
    if (selectedProp == null || selectedProp == "") {
      notifier.alert(
          'Datastream could not be created, at least one Observed Property is invalid');
      return false;
    }
    ...
    if (obj.name == null || obj.name == "") {
      notifier.alert(
          'Datastream could not be created, at least one Unit name is invalid');
      return false;
    }
    if (obj.symbol == null || obj.symbol == "") {
      notifier.alert(
          'Datastream could not be created, at least one Unit symbol is invalid');
      return false;
    }
    units.push(obj);
  }
  ...
  if (myDS.observation_types.includes(undefined) ||
      myDS.observation_types.includes(null)) {
    notifier.alert(
        'Datastream could not be created, at least one Observation Type is invalid');
    return false;
  }
  if (myDS.observedProperties.includes(undefined) ||
  ...
  $.ajax({
    ...
    error : function(response) {
      notifier.alert(
          'Datastream could not be created, check the Log for errors');
    ...
    success : function(e) {
      ...
      $("#datastreams")
          .find("select")
          .each(function() { $(this).append(option).trigger("change"); });
    },
  });
}
...
var streamTypes = [
  ...
  },
  {
    id : "1",
    text :
        "http://www.opengis.net/def/observationType/OGC-OM/2.0/OM_CategoryObservation (URL)",
  },
  {
    id : "2",
    text :
      ...
      .append(
          $("<tr>")
              ...
              .append($("<td>").append($("<select>")
                                           ...
                                           .attr("style", "width:150px")))
              .append($("<td>").append(
                  $("<input>").attr("size", 15).attr("type", "text")))
              .append($("<td>").append(
                  $("<input>").attr("size", 15).attr("type", "text")))
              .append($("<td>").append(
                  $("<input>").attr("size", 15).attr("type", "text")))
